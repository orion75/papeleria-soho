...
public class VendedorController {
    ...
    @Operation(summary = "Traer todos los Vendedores")
    @ApiResponses(value = {
    ...
    public ResponseEntity<?> get(@PathVariable("id") Long id) {
        return ResponseEntity.ok(terceroService.findById(id, ETipoTercero.VENDEDOR));
    }

    @GetMapping()
    public ResponseEntity<?> getAll() {
        return ResponseEntity.ok(terceroService.findAll(ETipoTercero.VENDEDOR));
    }

    @PostMapping()
    public ResponseEntity<?> save(@Valid @RequestBody TerceroRequest model) {
        return ResponseEntity.ok(terceroService.save(model, ETipoTercero.VENDEDOR));
    }
